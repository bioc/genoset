...
head(end(genoset.ds))
head(pos(genoset.ds)) # Floor of start, end midpoint
tail(pos(genoset.ds))
...
boundingIndices( c(127e6,127.5e6), c(127e6,128e6), pos(chr12.ds) )
...
amount of memory required. Note how the segmented values
...
@ 
Other segmenting methods could also be used. See "segs2Rle",
...
# Or use compress it to a DataFrame of Rle
# ~1/3 space used on random data
