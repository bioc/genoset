...
class is a ``locData'' slot that holds a GRanges object from the
GenomicRanges package or a RangedData object from the
...
been added.  This slot holds a GRanges object or a RangedData object that keeps track of the feature positions and
allows for quick subsetting. The RangedData is always sorted such that the chromosomes
...
information from a genoset object. Methods for RangedData and GRanges have also
been added to give them same API. We can access per-probe information
...
index can be a set of ranges. chrIndices with a chromosome argument is
...
example, ExpressionSet has the ``exprs'' function. Likewise, BAFSet
has ``baf'' and ``lrr'' and CNSet has ``cn''. It is common to put
...
this RangedData will prohibit mixing rows from different chromosomes. The function ``isGenomeOrder'' can check if
a RangedData or GRanges object, like the locData slot, is in genome order. Here
...
Additionally, the function readGenoSet takes care of re-attaching all
big.matrix objects to their on-disk components. readGenoSet takes the
