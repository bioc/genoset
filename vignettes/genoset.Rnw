...
The \Rpackage{genoset} package offers an extension of the familiar Bioconductor \Rclass{eSet} object for genome
assays: the \Rclass{GenoSet} class. The \Rclass{GenoSet}
...
In typical Bioconductor style, \Rclass{GenoSet} objects, and derivatives, can be created using the functions
...
elements can be matrices or \Rclass{DataFrames} with \Rclass{Rle} columns (from \Rpackage{IRanges}). They can also
be \Rclass{BigMatrix} or \Rclass{BigMatrixFactor} objects (from \Rpackage{bigmemoryExtras}).
...
Note that \Rfunction{names} lists the data matrices.

\subsection{Accessing Genome Information}
Now lets look at some special functions for accessing genome
information from a genoset object. These functions are all defined for \Rclass{GenoSet}, \Rclass{RangedData}
and \Rclass{GRanges} objects. We can access per-feature information
as well as summaries of chromosome boundaries in base-pair or
row-index units.

There are a number of functions for getting
portions of the locData data. \Rfunction{chr} and \Rfunction{pos} return the
chromosome and position information for each feature. \Rfunction{genoPos} is like \Rfunction{pos}, but it
...
chromosomes. \Rfunction{chrInfo} returns the genoPos of the first and last feature on each chromosome in
addition to the offset of the first feature from the start of the genome. \Rfunction{chrInfo} results are
used for drawing chromosome boundaries on genome-scale plots. \Rfunction{pos}
and \Rfunction{genoPos} are defined as the floor of the average of each
...
\Rclass{GenoSet}, \Rclass{GRanges}, and \Rclass{RangedData} objects can be set to, and checked for, genome
...
Certain methods on \Rfunction{GenoSet} objects expect the rows to be in genome
order. Users are free to rearrange rows within
chromosome as they please, although if the locData is a \Rfunction{RangedData},
...
\Rfunction{chrOrder} function. The object creation method \Rfunction{Genoset} creates objects in strict genome order. 
...
index can be a set of ranges or the usual logical, numeric or character indices. \Rfunction{chrIndices} with a chromosome argument is
...
\Rclass{eSet}-derived classes tend to have special functions to get and set
specific assayDataElement members (the big data matrices). For
example, \Rclass{ExpressionSet} has the \Rfunction{exprs} function. It is common to put
other optional matrices in assayData too (genotypes, quality
scores, etc.). These can be get and set with the \Rfunction{assayDataElement}
function, but typing that out can get old.  \Rclass{GenoSet} and derived
...
function \Rfunction{gcCorrect} can be used to remove this effect resulting in
...
\Rpackage{DNAcopy} package (the CBS algorithm).  Genoset adds features to split jobs among
processor cores. When the library \Rpackage{parallel} is loaded, the
argument n.cores can control the number of processor cores
utilized.

Additionally, \Rclass{GenoSet} stores segment values so that they can be
accessed quickly at both the feature and segment level. We use a
\Rclass{DataFrame} object from \Rpackage{IRanges} where each column is a
Run-Length-Encoded \Rclass{Rle} object. This dramatically reduces the
...
Or use the convenience function \Rfunction{runCBS}
<<runCBS>>=
gs[,,"cn.segs"] = runCBS(gs[,,"cn"],locData(gs))
@ 

Try it with \Rpackage{parallel}
...
representations of the segments. \Rpackage{Genoset} describes data in genome
...
representations can be used like regular vectors, plotted as segments (see \Rfunction{genoPlot}), and 
stored efficiently. A collection of \Rclass{Rle} objects, one for each sample, are often stored as 
one \Rclass{DataFrame} in a \Rclass{GenoSet}.  \Rpackage{Genoset} provides functions to quickly flip back and forth 
...
\Rfunction{segPairTable} summarizes two Rle objects into segments that have one unique 
...
\Rfunction{bounds2Rle} is convenient if you already know the genome feature indices corresponding 
to the bounds of each segment.

Currently we use \Rclass{data.frames} for tables of segments. In the near future these will have colnames 
that will make it easy to coerce these to \Rclass{GRanges}. Coercion to \Rclass{GRanges}
...
exons, genes or cytobands. The function \Rfunction{rangeSampleMeans} serves this
purpose. Given a \Rclass{RangedData} or \Rclass{GRanges} of arbitrary genome ranges and a
\Rclass{GenoSet} object, \Rfunction{rangeSampleMeans} will return a matrix of values
with a row for each range. 

\Rfunction{rangeSampleMeans} uses \Rfunction{boundingIndicesByChr} to select the features 
...
\Rfunction{rangeColMeans} is used to do a fast average of each of a
...
\Rclass{DataFrame} of \Rclass{Rle}, like that returned by \Rfunction{runCBS}.
...
argument for chromosome subsetting is provided. Like \Rfunction{plot}, \Rfunction{genoPlot} plots x against y. 
'x' can be some form of location data, like a \Rclass{GenoSet}, \Rclass{RangedData}, or\Rclass{GRanges}. 'y' is some 
form of data along those coordinates, like a numeric vector or \Rclass{Rle}. \Rfunction{genoPlot} marks 
...
Plot data without a \Rclass{GenoSet} object using numeric or \Rclass{Rle} data:
...
\section{\Rclass{CNSet} and \Rclass{BAFSet} Objects}
Two classes extend \Rclass{GenoSet}: \Rclass{CNSet} and \Rclass{BAFSet}.  \Rclass{CNSet} is the basic copy number object. It
requires that one assayDataElement be called ``cn'' slot, similar to the ExpressionSet uses ``exprs''.
\Rclass{BAFSet} is intended to store ``LRR'' or Log-R Ratio and ``BAF'' or B-Allele Frequency data for
...
of signal coming from the ``B'' allele, relative to the ``A'' allele, where A and B are arbitrarily assigned. BAF has the
...
copy-neutral LOH.  \Rclass{BAFSet}s require 'lrr' and 'baf' matrices as assayDataElements and have getter/setter 
methods for these elements.

\subsection{Processing B-Allele Frequency Data}
...
<<mbafcutoff>>=
baf.ds = GenoSet( locData=locData.rd, lrr=fake.lrr, baf=fake.baf, pData=fake.pData, annotation="SNP6" )
...
sample at a time if your data ``matrix'' is a \Rclass{DataFrame}.
...
\Rfunction{segTable} convenient for getting your data formatted for input. I 
find it convenient to load GISTIC output as a RangedData for 
intersection with gene locations.

\section{Big Data and \Rpackage{bigmemoryExtras}}
Genome-scale data can be huge and keeping everything in memory can get
you into trouble quickly, especially if you like using \Rpackage{parallel}'s \Rfunction{mclapply}.

It is often convenient to use \Rclass{BigMatrix} objects from the \Rpackage{bigmemoryExtras}
package as assayDataElements, rather than base matrices.  \Rclass{BigMatrix} is
based on the \Rpackage{bigmemory} package, which provides a matrix API to
...
RAM. The \Rpackage{bigmemoryExtras} vignette has more details about using \Rclass{eSet}-derived
classes and \Rclass{BigMatrix} objects.
