...
In typical Biocondutor style, GenoSet objects can be created using the functions
...
rle.ds = GenoSet( locData=locData.rd,
  cn.raw = fake.cn,
  cn.segments=DataFrame(K=Rle(c(rep(1.5,300),rep(2.3,700))),L=Rle( c(rep(3.2,700),rep(2.1,300)) ), M=Rle(rep(1.1,1000)),row.names=rownames(fake.lrr)),
  =DataFrame(K=Rle(c(rep(0.05,600),rep(0.5,400))),L=Rle( c(rep(0,700),rep(0.5,300)) ), M=Rle(rep(1,1000)),row.names=rownames(fake.baf)),
  pData=fake.pData,
  annotation="SNP6"
  )
@ 
...
It is often convenient to use BigMatrix objects from the BigMatrix
